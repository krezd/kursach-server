...
public class ProcessService {
    ...
    public ResponseEntity<?> addProcesses(UUID sessionId, List<Process> processes) {
        ...
        if (session != null) {
            List<Process> oldProcesses = processRepository.findAllBySessionId(sessionId);
                for (Process process : processes) {
                    Process matchingProcess = oldProcesses.stream()
                            .filter(p -> p.getProcessName().equals(process.getProcessName()))
                            .findFirst()
                            .orElse(null);
                    if (matchingProcess != null) {
                        matchingProcess.setTimeSpend(matchingProcess.getTimeSpend().plus(process.getTimeSpend()));
                        process.setProcessStatus(matchingProcess.getProcessStatus());
                        process.setTimeSpend(matchingProcess.getTimeSpend());
                        process.setId(matchingProcess.getId());
                        process.setSession(session);
                    } else {
                        process.setSession(session);
                        process.setProcessStatus(ProcessStatus.NEUTRAL);
                    }
